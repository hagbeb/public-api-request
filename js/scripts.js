...
function getModalPosition(element, position) {
    ...
    let newPosition = 0;
    // if we asked for the 'left' position, return left position, else return the right position
    if (position === 'left') {
        newPosition = rect.left + document.documentElement.scrollLeft;
    } else if (position === 'right') {
        // new right position is the page width minus the 'right' coordinate (since getBoundingClientRect returns coordinates...
        // ... relative to the left of the page, but to set 'right' position we want the number from the right of the page)
        newPosition = Number(document.body.offsetWidth) - Number(rect.right + document.documentElement.scrollLeft);   
    }
    return newPosition;
};
...
function replaceModal(element, replaceClass1, replaceClass2) {
    // remove the classes associated with non-current modals
    element.classList.remove(replaceClass1, replaceClass2, 'extra-modals');
...
function displayModal(person, buttonClicked) {
    ...
    // Display current person in modal:
        ...
        root.style.setProperty('--modal-right-position', `${getModalPosition(currentModal, 'right')}px`);
